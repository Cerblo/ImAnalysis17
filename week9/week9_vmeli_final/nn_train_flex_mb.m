...
% definining errors
err=zeros(nb_iter,1);
err_ep=zeros(floor(nb_iter/n),1);

%random init
for l=1:nb_hl
            weigths(l+1).layer=l;
            weigths(l+1).w=1/sqrt(nb_neurons_full(1,l))*rand(nb_neurons_full(1,l),nb_neurons_full(1,l+1));
            weigths(l).b=1/sqrt(nb_neurons_full(1,l))*rand(1,nb_neurons_full(1,l+1));
end
